package top.clearlight.bilibili.thread;

import java.util.concurrent.TimeUnit;

/**
 * volatile 关键字, 使一个变量在多个线程间可见
 * A B线程都用到一个变量, java默认是A线程中保留一份copy, 这样如果B线程修改了该变量, 则A线程未必知道
 * 使用volatile关键字, 会让所有线程都会读到变量的修改值
 * <p>
 * 在下面代码中, running是存在于堆内存的t对象中
 * 当线程t1开始运行的时候, 会把running值从内存中读到t1线程的工作区, 在运行过程中直接使用这个copy, 并不会每次都去读取堆内存,
 * 这样, 当主线程修改running的值之后, t1线程感知不到, 所以不会停止运行
 * <p>
 * 使用volatile, 当其中一个线程将值修改后,将会强制所有线程都去堆内存中读取running的值(写入之后,进行缓存过期通知)
 * <p>
 * 可以阅读这篇文章进行更深入的理解
 * https://www.cnblogs.com/nexiyi/p/java_memory_model_and_thread.html
 * <p>
 * volatile并不能保证多个线程共同修改running变量时锁带来的不一致问题, 也就是说volatile不能替代synchronized
 *
 * @date 2019.9.26
 */
public class Test8 {
    /* volatile*/ boolean running = true; // 对比一下有无volatile的情况, 整个程序运行结果的区别
    int i = 0;
    void m() {
        System.out.println("m start");
        while (running) {
            // 尽管running没有加volatile, 因为会执行里面的代码,有可能某一时间段会从主内存中读取数据
            System.out.println(i++);
        }
        System.out.println("m end");
    }

    public static void main(String[] args) {
        Test8 t = new Test8();

        new Thread(t::m, "t1").start();

        try {
            TimeUnit.SECONDS.sleep(1);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }

        t.running = false;
    }
}
